...
This module handles all database operations for the Video Preview Application.
It provides a centralized interface for storing, retrieving, and managing video
metadata in the SQLite database.

Features:
- Database initialization and schema management
- Duplicate detection using URLs and content hashing
...
class DatabaseHelper:
    ...
    def init_database(self) -> None:
        ...
        - Adds any missing columns for backward compatibility
    ...
    def is_duplicate(self, url: str, content_hash: str) -> bool:
        ...
        2. Content hash match (same video from different source)
    ...
    def save_to_database(self, video_info: Dict[str, Any]) -> Optional[int]:
        ...
        This method handles both new inserts and updates to existing entries
        based on the URL uniqueness constraint.
        
        Args:
            video_info: Dictionary containing video metadata
            
        Returns:
            int: ID of the new or updated record, or None if save failed
        """
        if not self.db_conn:
    ...
    def query_database(self, user: Optional[str] = None, year: Optional[int] = None, source: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        This method retrieves videos matching the specified filters.
        If no filters are provided, all videos are returned.
        ...
        Returns:
            List[Dict[str, Any]]: List of dictionaries containing video records
        """
        if not self.db_conn:
            logger.error("Database connection not available")
            return []
    ...
    def delete_video(self, video_id: int) -> bool:
        ...
        Note: This only removes the database entry, not the actual files.
    ...
    def get_video_by_id(self, video_id: int) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Dict[str, Any]: Dictionary with video data or None if not found
        """
        if not self.db_conn:
            logger.error("Database connection not available")
            return None
    ...
    def get_videos_by_user(self, username: str) -> List[Dict[str, Any]]:
        ...
        A convenience wrapper around query_database for the common
        case of filtering by username only.
    ...
    def close(self) -> None:
        ...
        or when the database connection is no longer needed.
        """
        if self.db_conn:
            self.db_conn.close()
            self.db_conn = None
